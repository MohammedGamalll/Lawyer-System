...
export function ArchivePage() {
  ...
  const [rows, setRows] = useState<{
    rows: { id: string; case_number: string; title: string }[];
  }>({ rows: [] });
  ...
  useEffect(() => {
    invoke<typeof rows>("cases:list", { pageSize: 50, archived: 1 })
      .then(setRows)
      .catch((e) => toast(e.message, "err"));
  }, []);
  const runLegacy = () =>
      ...
      <Card>
        ...
        <div className="mb-3 flex gap-2">
          <Input
            ...
            placeholder={t("searchPage.legacyHint")}
          />
        ...
        {(legacy.indexes || []).map((r) => (
          <div key={String(r.id)} className="border-b py-2 text-sm">
            {String(r.number || "")}/{String(r.year || "")} —{" "}
            {String(r.subject || r.entity || "")}
          </div>
        ))}
        {(legacy.rows || []).map((r) => (
          <div
            key={String(r.id)}
            className="border-b py-2 text-xs text-navy-600"
          >
            {String(r.source_file)} #{String(r.source_row)}{" "}
            {String(r.entity_hint || "")}
          </div>
        ))}
      </Card>
    </div>
