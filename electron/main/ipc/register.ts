...
export function registerIpc(ipc: IpcMain, getWin: () => BrowserWindow | null): void {
  ...
  handle(ipc, IPC.auth.resetPassword, { permission: 'users.manage', write: true }, (_e, user, userId, next) => {
    auth.resetPassword(user!, Number(userId), String(next))
    return ok(true)
  })

  handle(ipc, IPC.users.list, {}, (_e, _u, q) => ok(users.listUsers(q as never)))
  handle(ipc, IPC.users.get, { permission: 'users.manage' }, (_e, _u, id) => ok(users.getUser(Number(id))))
  handle(ipc, IPC.users.create, { permission: 'users.manage', write: true }, (_e, user, data) => ok(users.createUser(user!, data as never)))
  handle(ipc, IPC.users.update, { permission: 'users.manage', write: true }, (_e, user, id, data) => ok(users.updateUser(user!, Number(id), data as never)))
  handle(ipc, IPC.users.remove, { permission: 'users.manage', write: true }, (_e, user, id) => {
    users.removeUser(user!, Number(id))
    return ok(true)
  })
  handle(ipc, IPC.users.setPermissions, { permission: 'users.manage', write: true }, (_e, user, id, codes) => {
    users.setUserPermissions(user!, Number(id), codes as string[])
    return ok(true)
  })
  ...
  handle(ipc, IPC.settings.get, {}, () => ok(settings.getSettings()))
  ...
  handle(ipc, IPC.clients.get, { permission: 'clients.view' }, (_e, _u, id) => ok(clients.getClient(Number(id))))
  handle(ipc, IPC.clients.profile, { permission: 'clients.view' }, (_e, _u, id) => ok(clients.clientProfile(Number(id))))
  ...
  handle(ipc, IPC.clients.update, { permission: 'clients.update', write: true }, (_e, user, id, data) => ok(clients.updateClient(user!, Number(id), data as never)))
  handle(ipc, IPC.clients.remove, { permission: 'clients.delete', write: true }, (_e, user, id) => {
    clients.removeClient(user!, Number(id))
    return ok(true)
  })
  ...
  handle(ipc, IPC.clients.addContact, { permission: 'clients.update', write: true }, (_e, user, clientId, data) =>
    ok(people.addClientContact(user!, Number(clientId), data as never))
  )
  handle(ipc, IPC.clients.removeContact, { permission: 'clients.update', write: true }, (_e, _u, id) => {
    people.removeClientContact(Number(id))
    return ok(true)
  })
  ...
  handle(ipc, IPC.cases.get, { permission: 'cases.view' }, (_e, _u, id) => ok(cases.getCase(Number(id))))
  handle(ipc, IPC.cases.create, { permission: 'cases.create', write: true }, (_e, user, data) => ok(cases.createCase(user!, data as never)))
  handle(ipc, IPC.cases.update, { permission: 'cases.update', write: true }, (_e, user, id, data) => ok(cases.updateCase(user!, Number(id), data as never)))
  handle(ipc, IPC.cases.remove, { permission: 'cases.delete', write: true }, (_e, user, id) => {
    documents.deleteCaseDocumentsFromDisk(Number(id))
    cases.removeCase(user!, Number(id))
    return ok(true)
  })
  handle(ipc, IPC.cases.link, { permission: 'cases.update', write: true }, (_e, user, a, b, t) => {
    cases.linkCases(user!, Number(a), Number(b), String(t))
    return ok(true)
  })
  handle(ipc, IPC.cases.archive, { permission: 'cases.update', write: true }, (_e, user, id) => {
    cases.archiveCase(user!, Number(id), true)
    return ok(true)
  })
  handle(ipc, IPC.cases.restore, { permission: 'archive.view', write: true }, (_e, user, id) => {
    cases.archiveCase(user!, Number(id), false)
    return ok(true)
  })
  ...
  handle(ipc, IPC.hearings.update, { permission: 'hearings.update', write: true }, (_e, user, id, data) => ok(hearings.updateHearing(user!, Number(id), data as never)))
  handle(ipc, IPC.hearings.postpone, { permission: 'hearings.update', write: true }, (_e, user, id, date, time, reason) =>
    ok(hearings.postponeHearing(user!, Number(id), String(date), time as string, reason as string))
  )
  handle(ipc, IPC.hearings.remove, { permission: 'hearings.update', write: true }, (_e, user, id) => {
    hearings.removeHearing(user!, Number(id))
    return ok(true)
  })
  ...
  handle(ipc, IPC.lawyers.get, { permission: 'lawyers.view' }, (_e, _u, id) => ok(people.getLawyer(Number(id))))
  handle(ipc, IPC.lawyers.create, { permission: 'users.manage', write: true }, (_e, user, data) => ok(people.createLawyer(user!, data as never)))
  handle(ipc, IPC.lawyers.update, { permission: 'users.manage', write: true }, (_e, user, id, data) => ok(people.updateLawyer(user!, Number(id), data as never)))
  handle(ipc, IPC.lawyers.remove, { permission: 'users.manage', write: true }, (_e, user, id) => {
    people.removeLawyer(user!, Number(id))
    return ok(true)
  })
  handle(ipc, IPC.lawyers.dashboard, { permission: 'lawyers.view' }, (_e, _u, id) => ok(people.getLawyer(Number(id))))
  handle(ipc, IPC.lawyers.savePhoto, { permission: 'users.manage', write: true }, (_e, user, lawyerId, file) => {
    const dest = print.saveLawyerPhoto(file as { name: string; data: number[] })
    const current = people.getLawyer(Number(lawyerId))
    const l = current.lawyer as Record<string, unknown>
    people.updateLawyer(user!, Number(lawyerId), { ...l, photo_path: dest })
    return ok({ path: dest })
  })

  handle(ipc, IPC.employees.list, { permission: 'employees.view' }, (_e, _u, q) => ok(people.listEmployees(q as never)))
  handle(ipc, IPC.employees.get, { permission: 'employees.view' }, (_e, _u, id) => ok(people.getEmployee(Number(id))))
  ...
  handle(ipc, IPC.employees.update, { permission: 'employees.manage', write: true }, (_e, user, id, data) => ok(people.updateEmployee(user!, Number(id), data as never)))
  handle(ipc, IPC.employees.remove, { permission: 'employees.manage', write: true }, (_e, user, id) => {
    people.removeEmployee(user!, Number(id))
    return ok(true)
  })
  ...
  handle(ipc, IPC.opponents.get, { permission: 'opponents.view' }, (_e, _u, id) => ok(people.getOpponent(Number(id))))
  ...
  handle(ipc, IPC.opponents.update, { permission: 'opponents.manage', write: true }, (_e, user, id, data) => ok(people.updateOpponent(user!, Number(id), data as never)))
  handle(ipc, IPC.opponents.remove, { permission: 'opponents.manage', write: true }, (_e, user, id) => {
    people.removeOpponent(user!, Number(id))
    return ok(true)
  })
  handle(ipc, IPC.opponents.linkCase, { permission: 'opponents.manage', write: true }, (_e, user, opponentId, caseId) =>
    ok(people.linkOpponentToCase(user!, Number(opponentId), Number(caseId)))
  )
  ...
  handle(ipc, IPC.tasks.update, { permission: 'tasks.manage', write: true }, (_e, user, id, data) => ok(schedule.updateTask(user!, Number(id), data as never)))
  handle(ipc, IPC.tasks.remove, { permission: 'tasks.manage', write: true }, (_e, user, id) => {
    schedule.removeTask(user!, Number(id))
    return ok(true)
  })
  ...
  handle(ipc, IPC.reminders.update, { write: true }, (_e, user, id, data) => ok(schedule.updateReminder(user!, Number(id), data as never)))
  handle(ipc, IPC.reminders.dismiss, { write: true }, (_e, _u, id) => {
    schedule.dismissReminder(Number(id))
    return ok(true)
  })
  handle(ipc, IPC.reminders.remove, { write: true }, (_e, user, id) => {
    schedule.removeReminder(user!, Number(id))
    return ok(true)
  })
  ...
  handle(ipc, IPC.appointments.update, { permission: 'appointments.manage', write: true }, (_e, user, id, data) => ok(schedule.updateAppointment(user!, Number(id), data as never)))
  handle(ipc, IPC.appointments.remove, { permission: 'appointments.manage', write: true }, (_e, user, id) => {
    schedule.removeAppointment(user!, Number(id))
    return ok(true)
  })
  ...
  handle(ipc, IPC.calendar.move, { permission: 'calendar.view', write: true }, (_e, _u, kind, id, date, time) => {
    schedule.moveCalendarEvent(String(kind), Number(id), String(date), time as string)
    return ok(true)
  })
  ...
  handle(ipc, IPC.notifications.read, { write: true }, (_e, _u, id) => {
    schedule.markNotificationRead(Number(id))
    return ok(true)
  })
  ...
  handle(ipc, IPC.documents.remove, { permission: 'documents.delete', write: true }, (_e, user, id) => {
    documents.removeDocument(user!, Number(id))
    return ok(true)
  })
  handle(ipc, IPC.documents.open, { permission: 'documents.view' }, async (_e, _u, id) => ok(await documents.openDocument(Number(id))))
  handle(ipc, IPC.documents.versions, { permission: 'documents.view' }, (_e, _u, id) => ok(documents.documentVersions(Number(id))))
  handle(ipc, IPC.documents.rename, { permission: 'documents.upload', write: true }, (_e, user, id, title) =>
    ok(documents.renameDocument(user!, Number(id), String(title)))
  )
  handle(ipc, IPC.documents.move, { permission: 'documents.upload', write: true }, (_e, user, id, data) =>
    ok(documents.moveDocument(user!, Number(id), data as never))
  )
  handle(ipc, IPC.documents.download, { permission: 'documents.view' }, (_e, _u, id, versionId) =>
    ok(documents.downloadDocument(Number(id), versionId ? Number(versionId) : undefined))
  )
  ...
  handle(ipc, IPC.poa.update, { permission: 'poa.manage', write: true }, (_e, user, id, data) => ok(legal.updatePoa(user!, Number(id), data as never)))
  handle(ipc, IPC.poa.remove, { permission: 'poa.manage', write: true }, (_e, user, id) => {
    legal.removePoa(user!, Number(id))
    return ok(true)
  })
  ...
  handle(ipc, IPC.contracts.update, { permission: 'contracts.manage', write: true }, (_e, user, id, data) => ok(legal.updateContract(user!, Number(id), data as never)))
  handle(ipc, IPC.contracts.remove, { permission: 'contracts.manage', write: true }, (_e, user, id) => {
    legal.removeContract(user!, Number(id))
    return ok(true)
  })
  ...
  handle(ipc, IPC.consultations.update, { permission: 'consultations.manage', write: true }, (_e, user, id, data) => ok(legal.updateConsultation(user!, Number(id), data as never)))
  handle(ipc, IPC.consultations.remove, { permission: 'consultations.manage', write: true }, (_e, user, id) => {
    legal.removeConsultation(user!, Number(id))
    return ok(true)
  })
  ...
  handle(ipc, IPC.correspondence.update, { permission: 'correspondence.manage', write: true }, (_e, user, id, data) => ok(legal.updateCorrespondence(user!, Number(id), data as never)))
  handle(ipc, IPC.correspondence.remove, { permission: 'correspondence.manage', write: true }, (_e, user, id) => {
    legal.removeCorrespondence(user!, Number(id))
    return ok(true)
  })
  ...
  handle(ipc, IPC.payments.balance, { permission: 'accounts.view' }, (_e, _u, clientId, caseId) =>
    ok(finance.paymentBalance(clientId ? Number(clientId) : undefined, caseId ? Number(caseId) : undefined))
  )
  ...
  handle(ipc, IPC.payments.remove, { permission: 'accounts.payment', write: true }, (_e, user, id) => {
    finance.removePayment(user!, Number(id))
    return ok(true)
  })
  ...
  handle(ipc, IPC.expenses.remove, { permission: 'accounts.expense', write: true }, (_e, user, id) => {
    finance.removeExpense(user!, Number(id))
    return ok(true)
  })
  ...
  handle(ipc, IPC.invoices.get, { permission: 'invoices.view' }, (_e, _u, id) => ok(finance.getInvoice(Number(id))))
  ...
  handle(ipc, IPC.invoices.update, { permission: 'invoices.manage', write: true }, (_e, user, id, data) => ok(finance.updateInvoice(user!, Number(id), data as never)))
  handle(ipc, IPC.invoices.remove, { permission: 'invoices.manage', write: true }, (_e, user, id) => {
    finance.removeInvoice(user!, Number(id))
    return ok(true)
  })
  ...
  handle(ipc, IPC.cashbox.update, { permission: 'accounts.view', write: true }, (_e, _u, id, data) => ok(finance.updateCashbox(Number(id), data as never)))
  handle(ipc, IPC.cashbox.transactions, { permission: 'cashbox.view' }, (_e, _u, id, q) => ok(finance.cashboxTransactions(Number(id), q as never)))
  ...
  handle(ipc, IPC.audit.remove, { permission: 'audit.delete', write: true }, (_e, user, ids) => {
    ...
    const list = Array.isArray(ids) ? (ids as number[]) : [Number(ids)]
  ...
  handle(ipc, IPC.print.preview, { permission: 'invoices.view' }, (_e, _u, invoiceId) => {
    const data = getInvoice(Number(invoiceId))
  ...
  handle(ipc, IPC.print.receipt, { permission: 'accounts.view' }, (_e, _u, paymentId) =>
    ok(finance.receiptPrintPayload(Number(paymentId)))
  )
  handle(ipc, IPC.print.voucher, { permission: 'accounts.view' }, (_e, _u, expenseId) =>
    ok(finance.voucherPrintPayload(Number(expenseId)))
  )

  handle(ipc, IPC.updater.check, {}, async () => ok(await updater.checkUpdates()))
  handle(ipc, IPC.updater.version, { auth: false }, () => ok(updater.appVersion()))
  handle(ipc, IPC.demo.seed, { permission: 'settings.manage', write: true }, () => ok(demo.seedDemoData()))
}
