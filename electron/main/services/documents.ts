...
export function uploadDocument(
  ...
  const info = db
    .prepare(
      `INSERT INTO documents (title, category, client_id, case_id, hearing_id, contract_id, file_path, file_name, mime_type, file_size, current_version, notes, created_by, created_at, updated_at)
       VALUES (?,?,?,?,?,?,?,?,?,?,1,?,?,?,?)`
    )
    .run(
      meta.title,
      ...
      meta.client_id || null,
      meta.case_id || null,
      meta.hearing_id || null,
      meta.contract_id || null,
      dest,
      file.name,
      file.mime ?? null,
  ...
  db.prepare(
    `INSERT INTO document_versions (document_id, version, file_path, file_name, created_by, created_at) VALUES (?,?,?,?,?,?)`
  ).run(id, 1, dest, file.name, actor.id, ts)
  audit(actor, 'create', 'documents', id, `تم رفع المستند ${meta.title}`)
  return { id }
}

export function updateDocument(actor: AuthedUser, id: number, data: Record<string, unknown>, file?: { name: string; data: Buffer | Uint8Array; mime?: string }) {
  const db = getDb()
  const old = db.prepare('SELECT * FROM documents WHERE id = ?').get(id) as { current_version: number; title: string } | undefined
  ...
  if (file) {
    ...
    db.prepare(
      `INSERT INTO document_versions (document_id, version, file_path, file_name, created_by, created_at) VALUES (?,?,?,?,?,?)`
    ).run(id, version, dest, file.name, actor.id, nowIso())
  }
  db.prepare(
...
export function removeDocument(actor: AuthedUser, id: number) {
  const db = getDb()
  const doc = db.prepare('SELECT * FROM documents WHERE id = ?').get(id) as { file_path: string; title: string } | undefined
  if (!doc) throw new Error('المستند غير موجود')
  const versions = db.prepare('SELECT file_path FROM document_versions WHERE document_id = ?').all(id) as { file_path: string }[]
...
export function openDocument(id: number) {
  const doc = getDb().prepare('SELECT file_path FROM documents WHERE id = ?').get(id) as { file_path: string } | undefined
...
export function documentVersions(id: number) {
  return getDb().prepare('SELECT * FROM document_versions WHERE document_id = ? ORDER BY version DESC').all(id)
}
...
export function deleteCaseDocumentsFromDisk(caseId: number): void {
  const db = getDb()
  const docs = db.prepare('SELECT id FROM documents WHERE case_id = ?').all(caseId) as { id: number }[]
  for (const d of docs) {
    const versions = db.prepare('SELECT file_path FROM document_versions WHERE document_id = ?').all(d.id) as { file_path: string }[]
    for (const v of versions) {
      if (v.file_path && fs.existsSync(v.file_path)) fs.unlinkSync(v.file_path)
    }
  }
...
export function renameDocument(actor: AuthedUser, id: number, title: string) {
  if (!title?.trim()) throw new Error('العنوان مطلوب')
  const db = getDb()
  const old = db.prepare('SELECT title FROM documents WHERE id = ?').get(id) as { title: string } | undefined
  if (!old) throw new Error('المستند غير موجود')
...
export function moveDocument(actor: AuthedUser, id: number, data: { client_id?: number; case_id?: number; category?: string }) {
  const db = getDb()
  db.prepare('UPDATE documents SET client_id=COALESCE(?, client_id), case_id=COALESCE(?, case_id), category=COALESCE(?, category), updated_at=? WHERE id=?').run(
    data.client_id || null,
    data.case_id || null,
    data.category ?? null,
    nowIso(),
...
export function downloadDocument(id: number, versionId?: number) {
  const db = getDb()
  let filePath: string | undefined
  let fileName: string | undefined
  if (versionId) {
    const v = db.prepare('SELECT file_path, file_name FROM document_versions WHERE id = ?').get(versionId) as { file_path: string; file_name: string } | undefined
    filePath = v?.file_path
    fileName = v?.file_name
  } else {
    const d = db.prepare('SELECT file_path, file_name FROM documents WHERE id = ?').get(id) as { file_path: string; file_name: string } | undefined
